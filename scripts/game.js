...
;(function(game, cjs){
	game.eventHandler = {
		init: function(){
			game.stage.addEventListener('stagemousedown', this.mouseDownHandler);
			game.stage.addEventListener('stagemouseup', this.mouseUpHandler);
		},
		mouseDownHandler: function(event){
			event.preventDefault();
			if (event.stageX < game.setting.gameWidth / 2) {
				game.coffeecup.nowSpeed = -game.setting.cupSpeed;
			}else{
				game.coffeecup.nowSpeed = game.setting.cupSpeed;
			}
			return false;
	...
	game.coffeecup = {
		...
		init: function(){	
			...
			this.boundaryL = - game.userCup.getBounds().width * game.userCup.scaleX,
			this.boundaryR = game.setting.gameWidth + this.boundaryL;
		},
		tick: function(event){
			if (!cjs.Ticker.paused) {
				var deltaS = event.delta / 1000,
				    toBe = game.userCup.x + deltaS * this.nowSpeed;

				if (toBe > this.boundaryR) {
					game.userCup.x = this.boundaryR;
				}else if (toBe < 0) {
					game.userCup.x = 0;
				}else{
					game.userCup.x = toBe;
